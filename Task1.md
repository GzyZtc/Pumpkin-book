...
<h3>ROC与AUC</h3>
ROC曲线（Receiver Operating Characteristic）是二元分类器性能评估的常用工具。它是画在坐标系上的，将真正例（True Positive Rate）与假正例（False Positive Rate）作为坐标轴的曲线。ROC曲线可以帮助我们确定一个最佳的阈值，使得分类器的性能尽可能优秀。
<br>
AUC是ROC曲线下面积，给出了二元分类器的整体性能。AUC值越接近1，说明分类器的性能越好，值越接近0，说明分类器的性能越差。AUC值为0.5时，说明分类器的性能等同于随机猜测。<br>
