...
>定义:过拟合：给定一个假设空间F，一个假设f属于F，如果存在其他的假设f'也属于F，使得在训练集上f的损失比f'小。但在整个样本空间上f'的损失比f的损失小，那么就说假设f过度拟合训练数据
...
<h3>ROC与AUC</h3>
ROC曲线（Receiver Operating Characteristic）是二元分类器性能评估的常用工具。它是画在坐标系上的，将真正例（True Positive Rate）与假正例（False Positive Rate）作为坐标轴的曲线。ROC曲线可以帮助我们确定一个最佳的阈值，使得分类器的性能尽可能优秀。<br>

其中，曲线纵轴真正例率(TPR)和横轴假正例率(FPR)的定义为
...
AUC是ROC曲线下面积，给出了二元分类器的整体性能

假设ROC曲线是由坐标为$\{(x_1,y_1),...(x_m,y_m)\}$的点按序连接而形成$(x_1=0,x_m=1)$，则AUC可估算为

$$AUC=\frac{1}{2}\overset{m-1}{\underset{i=1}{\sum}}(x_{i+1}-x_i)(y_i+y_{i+1})$$
...
AUC值越接近1，说明分类器的性能越好，值越接近0，说明分类器的性能越差。AUC值为0.5时，说明分类器的性能等同于随机猜测。<br>

AUC是一个无单位的量，可以用来评估不同分类器的性能，甚至不同特征集合的性能。所以AUC是一个很常用的二元分类器的性能评估指标。<br>
ROC曲线可以帮助我们确定一个最佳的阈值，使得分类器的性能尽可能优秀。曲线下面积（AUC）越大，说明分类器的性能越好。
<h3>代价敏感错误率与代价曲线</h4>
<h2>比较检验，假设检验</h2>
<h2>偏差与方差</h2>
